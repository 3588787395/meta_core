#!/usr/bin/env python3
"""Check 4 复核补充验证：修正假阴性并覆盖模式切换、TDX 专用导入等。"""
import json
import time
import urllib.error
...
def jreq(method, path, body=None, params=None):
    if params:
        path += "?" + "&".join(f"{k}={urllib.parse.quote(str(v))}" for k, v in params.items())
    st, raw, _ = req(method, path, body)
...
def main():
    ...
    record("DZH XML import-and-save", ok, f"status={st}, nodes={d.get('meta',{}).get('node_count')}, edges={d.get('meta',{}).get('edge_count')}")

    # 2. DZH export (functional: valid XML bytes, ignore missing Content-Type)
    if dzh_cfg:
        ...
        record("DZH XML export-meta", ok2, f"status={st}, bytes={len(raw2)}")
    else:
        record("DZH XML export", False, "no config")
        record("DZH XML export-meta", False, "no config")

    # 3. TDX import via dedicated /api/dzh/tdx/import
    ...
    record("TDX XML import (/api/dzh/tdx/import)", st == 200 and dtdx.get("success"),
           f"status={st}, success={dtdx.get('success')}, cells={dtdx.get('stats',{}).get('cells')}")

    # 4. TDX import via auto-detect /api/dzh/import
    ...
    # 5. JSON import with a valid new-format file (roundtrip)
    valid_json = {
        "version": 2,
    ...
    # 9. Sim start + wait init + pause/resume/step/stop
    sim_cfg = json_cfg if json_cfg else dzh_cfg
    st, d = jreq("POST", "/api/sim/start", {"config": sim_cfg, "speed": 100.0})
    ...
    if sim_sid:
        ...
        st, d = jreq("POST", "/api/sim/control", {"session_id": sim_sid, "action": "pause"})
        record("Sim pause", st == 200 and d.get("code") == 0, f"status={st}, code={d.get('code')}, msg={d.get('msg')}")

        st, d = jreq("POST", "/api/sim/control", {"session_id": sim_sid, "action": "resume"})
        record("Sim resume", st == 200 and d.get("code") == 0, f"status={st}, code={d.get('code')}, msg={d.get('msg')}")

        # step 可能较重，这里仅做轻量验证：使用小步长并忽略超时
        try:
            st, d = jreq("POST", "/api/sim/control", {"session_id": sim_sid, "action": "step", "params": {"delta": 1.0}})
            record("Sim step", st == 200 and d.get("code") == 0, f"status={st}, code={d.get('code')}")
        except Exception as e:
            record("Sim step", False, f"异常/超时: {e}")

        st, d = jreq("POST", "/api/sim/control", {"session_id": sim_sid, "action": "stop"})
        record("Sim stop", st == 200 and d.get("code") == 0, f"status={st}, code={d.get('code')}")

    # 10. Mode after sim
    st, d = jreq("GET", "/api/state/runtime")
    after_sim_mode = d.get("mode", "unknown") if st == 200 else "unknown"
    record("Runtime state (after sim stop)", st == 200, f"status={st}, mode={after_sim_mode}")

    # 11. Replay start + pause/stop
    st, d = jreq("POST", "/api/replay/start", {"config": sim_cfg, "speed": 1000, "base_period": "day"})
    rep_ok = st == 200 and d.get("code") == 0
    rep_sid = d.get("data", {}).get("session_id") if rep_ok else None
    record("Replay start", rep_ok, f"status={st}, session_id={rep_sid}")

    if rep_sid:
        st, d = jreq("POST", "/api/replay/control", {"session_id": rep_sid, "action": "pause"})
        record("Replay pause", st == 200 and d.get("code") == 0, f"status={st}, code={d.get('code')}")
        st, d = jreq("POST", "/api/replay/control", {"session_id": rep_sid, "action": "stop"})
        record("Replay stop", st == 200 and d.get("code") == 0, f"status={st}, code={d.get('code')}")

    # 12. Mode after replay
    st, d = jreq("GET", "/api/state/runtime")
    after_rep_mode = d.get("mode", "unknown") if st == 200 else "unknown"
    record("Runtime state (after replay stop)", st == 200, f"status={st}, mode={after_rep_mode}")
