...
## [x] Task 9: 事件面板重构（可视化版）
- **Priority**: high
- **Depends On**: Task 6, Task 7
- **Description**:
  - 重新设计事件面板为可视化面板：顶部显示事件分类矩阵（每类一行图标）或散点分布视图
  - 分类矩阵中每行显示分类图标、名称、计数，右侧显示该分类下最近的事件图标流（已发生=实心，排队中=虚线动画）
  - 点击分类行或单个事件图标，在下方详情区域显示相关事件记录文本
  - 独立定时器队列区域：显示排队中的 timer 事件及其预计触发时间
  - 保留拖拽浮窗、折叠/展开、暂停/继续接收、清空事件功能
  - 9 种事件分类：Tick、Bar、Formula、Edge、Transfer、Signal、Order、TTL、System
- **Acceptance Criteria Addressed**: AC-4, AC-11
- **Test Requirements**:
  - `programmatic` TR-9.1: WebSocket/SSE 事件能正确接收并显示在分类矩阵中
  - `programmatic` TR-9.2: 分类矩阵每行正确显示该分类的事件图标（含排队中状态）
  - `programmatic` TR-9.3: 点击分类行在下方详情区显示该分类所有事件文本记录
  - `programmatic` TR-9.4: 点击单个事件图标在下方详情区显示该事件及相关事件文本
  - `programmatic` TR-9.5: 定时器队列正确显示排队中的 timer 事件及预计触发时间
  - `programmatic` TR-9.6: 散点分布视图按时间和分类正确绘制事件点
  - `human-judgment` TR-9.7: 事件面板拖拽位置保存到本地存储，折叠/展开状态保存
