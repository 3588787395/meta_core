...
* **Test Requirements**:
  ...
  * `programmatic` TR-1.1: 画布正确渲染示例池拓扑（source→cond1→pool\_A→cond3→pool\_C, source→cond2→pool\_B→cond3）

  * `programmatic` TR-1.2: 节点可拖拽移动，画布缩放正常

  * `human-judgement` TR-1.3: 线形切换效果正确，连线模式操作流畅
...
* **Test Requirements**:
  ...
  * `human-judgement` TR-2.3: 复制/剪切/粘贴操作符合预期，边属性设置正确
...
* **Description**:
  ...
  * 实现字段联动（depends\_on/active\_when）
...
* **Test Requirements**:
  ...
  * `human-judgement` TR-3.3: 字段联动逻辑正确，校验错误提示清晰
...
* **Test Requirements**:
  ...
  * `human-judgement` TR-4.2: 字段编辑器功能完整，批量编辑便捷
...
* **Test Requirements**:
  ...
  * `human-judgement` TR-8.2: 事件分类筛选正确，图标颜色清晰
...
* **Notes**: 使用 integrated\_browser MCP 工具
