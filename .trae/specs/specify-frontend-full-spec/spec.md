# Meta Core 股票池平台 - 前端完整功能规范

## Overview

### Summary
本规范基于对现有代码的全面分析，详细定义 Meta Core 股票池平台前端的所有功能模块、处理流程、状态管理和交互逻辑。前端采用单页应用架构，包含三个核心视图（主页、配置中心、公式管理），基于表驱动设计原则构建，所有组件类型和交互行为由后端配置表动态决定。
...
| 功能模块 | 主要文件 | 核心类/对象 |
|---------|---------|------------|
| 画布引擎 | `web/js/canvas.js` | `FlowCanvas`, `VirtualScroller`, `DZH_COLOR_UTILS` |
...
| Tick | 📊 | 蓝色 | DataChanged |
| Bar | 📈 | 绿色 | BarComposed |
| Formula | 🧮 | 紫色 | FormulaEvaluated, StockFiltered |
| Edge | ⚡ | 橙色 | EdgeFired, CrossOver |
| Transfer | 🔄 | 青色 | TransferExecuted, Executed |
| Signal | 💰 | 红色 | BUY, SELL |
| Order | 📋 | 灰色 | OrderPlaced, OrderFilled, PositionUpdated |
| TTL | ⏰ | 黄色 | TTLExpired, Timeout |
| System | 🔧 | 白色 | ModeChanged, TimeAdvanced, PoolLoaded |
...
**处理流程：**
```
WebSocket接收事件 → EventPanel.addEvent(event) → 根据类型分配颜色和图标 → 渲染事件条目 → 自动滚动(如果启用)
```
...
- **Given**：事件面板已打开
- **When**：用户拖拽、折叠、筛选事件
- **Then**：拖拽位置保存到本地存储，折叠/展开状态保存，筛选功能正常
- **Verification**：`human-judgment`
...
| 虚拟滚动 | `web/js/canvas.js` | `VirtualScroller` | `setData()`, `_renderVisibleRows()` |
...
| 事件面板 | `web/js/ui.js` | `EventPanel` | `addEvent()`, `clear()`, `filter()`, `toggleAutoScroll()` |
