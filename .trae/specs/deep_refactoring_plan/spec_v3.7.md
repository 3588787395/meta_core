...
| `formula_router` | 公式路由器，双引擎公式求值入口 | `engine.py:400` |
| `_rt` | 运行时状态容器，存放 node_stocks 等动态计算状态 | `engine.py:349` |
| `_compiled_cache` | 编译期调度表缓存，{pool_config哈希: CompiledSchedule} | `engine.py:460` |
| `highlight_events` | 高亮事件类型列表，对外事件订阅的配置入口 | `engine.py:371` |
...
## 3. 类2：FormulaRouter —— 公式路由器，双引擎路由+缓存管理
...
| 属性 | 含义 | 代码引用 |
|------|------|---------|
| `cache` | 公式结果缓存（FormulaCache 实例） | `formula_router.py:101` |
| `engines` | 可用引擎集合（python 始终可用，hqchart 视环境而定） | `formula_router.py:112` |
| `_routing_rules` | 引擎路由规则表（表驱动，按复杂度/周期决策） | `formula_router.py:106` |
| `_simple_functions` | 简单函数白名单，用于复杂度判定 | `formula_router.py:103` |
...
| 属性 | 含义 | 代码引用 |
|------|------|---------|
| `tables` | 配置表集合，{表名: 表数据dict}（通过 get() 访问） | `table_engine.py:32` |
| `validators` | 自定义校验器注册表 | `table_engine.py:35` |
| `config_dir` | 配置表所在目录路径 | `table_engine.py:31` |
| `_schema_validator` | 三级校验器引用 | `table_engine.py:38` |
...
## 8. 股票池本质一句话概括（v3.7版，28字）

**股票池是行情驱动的有向图过滤器，股票经边公式筛选在节点间流转。**
