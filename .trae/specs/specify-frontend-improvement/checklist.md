...
- [ ] **Check 4: 股票池功能正确且简洁（权重 25）**
  - [ ] 节点增删改查（备选池、转移条件、状态池、丢弃池、文字标签）工作正常（本次静态审查未验证）
  - [ ] 连线增删改查与属性（条件/无条件、线形、描述文字、线条宽度）工作正常（本次静态审查未验证）
  - [ ] 右键菜单（添加节点、属性、复制/剪切/粘贴、层级、删除、综合设置）工作正常（本次静态审查未验证）
  - [ ] 导入导出（DZH XML、TDX XML、JSON）工作正常（本次静态审查未验证）
  - [ ] 运行控制（开始、暂停、停止）与模式切换（设计/仿真/回放/实盘）工作正常（本次静态审查未验证）
...
- [ ] **Check 5: 唯一正确路径，无多路径/兼容/特殊处理（权重 15）**
  ...
  - [ ] 仿真/回放/实盘模式共享同一条执行路径，无特殊分支（本次静态审查未完整验证）
  - [x] 重复全局钩子（如 `window.timelineAddEvent`、`window.logSystemEvent`）已收敛
  - [ ] 未使用的变量、函数、DOM 引用、CSS 类已清理（`startTimerPolling`/`stopTimerPolling` 当前为未调用死代码）
...
| 5     | 15   | 15   | 9    | 否       |
| 6     | 15   | 15   | 0    | 否       |
| **Total** | **110** | **100** | **70.78** | **否** |

> 注：权重总和为 110，标准化后总分为 100。计算方式：单项贡献 = 权重 × (子条款通过数 / 子条款总数)，总分 = 所有单项贡献之和 / 1.1。本次 Task 3 相关检查点已通过，但 Check 4–6 尚未完成，整体仍不满足进入下一任务的条件。
