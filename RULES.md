...
120. **所有事件 handler 必须 @_event_handler 装饰 + 审计盲区闭合后的全局收敛上限**：所有通过 `self._bus.subscribe()` 注册的事件 handler 必须使用 `@_event_handler` 装饰器（禁止裸 handler），防止 handler 异常中断 EventBus.publish 同步扇出链——一个 handler 抛未捕获异常会导致后续订阅者不执行，事件链断裂。`@_event_handler`（event_bus.py:15-26）将异常处理从 handler 体内部上提到装饰器层（AOP 横切），统一"handler 不应中断事件链"的运行时契约（捕获→logger.warning→返回 None）。这是「极致本质的运行时」的第十层洞察：异常处理覆盖完整性是运行时安全本质。**v12 彻底性修正**：v11 的「审计盲区闭合后的全局上限」声明再次是审计盲区制造的假象——v11 上限仅适用于 v11 已闭合盲区（`core/` + `converters.py`），但残留盲区使全局上限未真正成立：v11 删除 DZHPoolExecutor 轮询基础设施但保留 execute_once，同时把 /pool/run 改委托 PoolEngine.execute_pool，导致 execute_once 失去唯一调用方，DZHPoolExecutor 整个类变为死代码；metatest 检测器有 4 处缺陷（threading.Timer 无检测器 / asyncio.sleep 正则方向反转 / NAME-BASED 检测换名漏检 / 字符串匹配 docstring 假阳性）；services/*.py 盲区同源（check #23/#25 只扫 core/ 不扫 services）。v12 闭合这些残留后全局收敛上限声明才真正成立——清理自身残留 + 检测器硬化后的全局收敛上限。
...
122. **清理自身残留**：删除平行运行时或重复实现时，必须同时删除失去调用方的残留入口与传递死代码（仅被死类实例化的 helper 类）。禁止保留 `execute_once` 等孤儿方法作为「未来可能用到的 thin wrapper」——若当前零调用方，即死代码。metatest 死代码检测必须用 AST 引用计数（排除 docstring/注释/字符串字面量），不得用字符串正则匹配。这是「极致本质的运行时」的第十二层洞察：清理自身的残留是收敛上限的下一个敌人——v11 删除 DZHPoolExecutor 轮询基础设施但保留 execute_once，同时把 /pool/run 改委托 PoolEngine.execute_pool，导致 execute_once 失去唯一调用方，DZHPoolExecutor 整个类（含 execute_once / NodeStateMachine / _init_nodes_edges / _init_mock_stocks / _build_mock_tick / _normalize_edge）变为 ~200~250 行传递死代码，但 v11 的 _collect_dead_code_violations 用字符串正则 `\bClassName\b` 匹配 ref_blob，metatest docstring 假阳性致漏判。v12 整个类删除 + AST 引用检测闭合此残留。
